...
export async function generateMetadata({
  ...
  return { title: `${t("commercial")} | Sweet Estate` };
}
...
export default async function CommercialPage({
  ...
  const sanity = await safeGetPropertiesByType("commercial", locale);
  const sourceProperties =
    sanity.length > 0
      ? sanity
      : mockProperties.filter((p) => p.type === "commercial");
    ...
    <div className="pt-16 md:pt-[72px] min-h-screen bg-white">
      <div className="bg-[#f8f8f8] border-b border-[#ebebeb]">
        <div className="container-site py-8 md:py-10">
          <h1 className="text-2xl md:text-3xl font-bold text-[#171717] mb-1">{t("commercial")}</h1>
